...
def build(input_reader_config,
    ...
    """
    if not isinstance(input_reader_config, input_reader_pb2.InputReader):   # 判断一个对象是否是一个已知的类型
        raise ValueError('input_reader_config not of type '
                         'input_reader_pb2.InputReader.')
    dataset = dataset_builder.build(input_reader_config, model_config,
                                    training, voxel_generator, target_assigner) # 获取数据
    dataset = DatasetWrapper(dataset)   # 封装数据
    return dataset
