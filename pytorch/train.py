...
import os, sys
sys.path.append("/home/yml/second.pytorch/")
...
def train(config_path,  #导入的config文件
          model_dir,#生成结果的路径
          result_path=None,
          create_folder=False,
          display_step=50,
    ...
    with open(config_path, "r") as f:
        proto_str = f.read()
        text_format.Merge(proto_str, config)  #将proto_str融合到一个config之中
    ...
    class_names = list(input_cfg.class_names)   #car
    ######################
    ...
    target_assigner = target_assigner_builder.build(target_assigner_cfg,
                                                    bv_range, box_coder) #把box_coder变成了target_assigner的一部分
    ######################
    # BUILD NET
    ######################
    ...
    gstep = net.get_global_step() - 1  #20698
    optimizer_cfg = train_cfg.optimizer
    if train_cfg.enable_mixed_precision:
        net.half()
        net.metrics_to_float()
        net.convert_norm_to_float(net)
    optimizer = optimizer_builder.build(optimizer_cfg, net.parameters())  #Adam
    if train_cfg.enable_mixed_precision:
...
# 生成数据库
    ...
    def _worker_init_fn(worker_id):
        ...
        np.random.seed(time_seed + worker_id) # worker是随机生成的
        print(f"WORKER {worker_id} seed:", np.random.get_state()[1][0])
# 获取数据的工具
    dataloader = torch.utils.data.DataLoader(
        dataset,
        batch_size=input_cfg.batch_size,  # 2
        shuffle=True,  #  数据洗牌
        num_workers=input_cfg.num_workers, # 2
        pin_memory=False,
    ...
    remain_steps = train_cfg.steps - net.get_global_step()  # 296960-
    t = time.time()
    ckpt_start_time = t

    total_loop = train_cfg.steps // train_cfg.steps_per_eval + 1   # 33
    ...
    try:
        for _ in range(total_loop):
            ...
            for step in range(steps):
                lr_scheduler.step()
                try:
                    example = next(data_iter)  #example  每次数量
                except StopIteration:
                    print("end epoch")
                    if clear_metrics_every_epoch:
                        net.clear_metrics()
                    data_iter = iter(dataloader)
                    example = next(data_iter)   # 查看导入数据确实是每次随机的
